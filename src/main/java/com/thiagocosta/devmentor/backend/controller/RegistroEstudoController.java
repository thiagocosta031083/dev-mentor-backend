...
public class RegistroEstudoController {
    ...
    @PostMapping
    public ResponseEntity<RegistroEstudoResponseDTO> criar(
            @Valid @RequestBody RegistroEstudoRequestDTO request, Principal principal) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new RegistroEstudoResponseDTO(service.criar(request, principal.getName())));
    }
