...
public class PlanoEstudoController {
    ...
    public PlanoEstudoController(PlanoEstudoService service) {
        this.service = service;
    }

    @PostMapping
    public ResponseEntity<PlanoEstudoResponseDTO> criar(
            @Valid @RequestBody PlanoEstudoRequestDTO request, Principal principal) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new PlanoEstudoResponseDTO(service.criar(request, principal.getName())));
    }
    ...
    public List<PlanoEstudoResponseDTO> listar(
            @PathVariable Long tecnologiaId, Principal principal) {
        return service.listar(tecnologiaId, principal.getName()).stream()
                .map(PlanoEstudoResponseDTO::new)
                .collect(Collectors.toList());
    }

    @PutMapping("/{id}")
    public PlanoEstudoResponseDTO atualizar(
            @PathVariable Long id,
            @Valid @RequestBody PlanoEstudoRequestDTO request,
            Principal principal) {
