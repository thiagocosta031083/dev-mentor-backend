...
public class ProjetoPessoalController {
    ...
    public ProjetoPessoalController(ProjetoPessoalService service) {
        this.service = service;
    }

    @PostMapping
    public ResponseEntity<ProjetoPessoalResponseDTO> criar(
            @Valid @RequestBody ProjetoPessoalRequestDTO request, Principal principal) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ProjetoPessoalResponseDTO(service.criar(request, principal.getName())));
    }

    @GetMapping
    public List<ProjetoPessoalResponseDTO> listar(Principal principal) {
        return service.listar(principal.getName()).stream()
                .map(ProjetoPessoalResponseDTO::new)
    ...
    @PutMapping("/{id}")
    public ProjetoPessoalResponseDTO atualizar(
            @PathVariable Long id,
            @Valid @RequestBody ProjetoPessoalRequestDTO request,
            Principal principal) {
