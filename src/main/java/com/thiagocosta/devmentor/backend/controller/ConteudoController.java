...
public class ConteudoController {
    ...
    public ResponseEntity<ConteudoResponseDTO> criar(
            @Valid @RequestBody ConteudoRequestDTO request, Principal principal) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ConteudoResponseDTO(service.criar(request, principal.getName())));
    }
    ...
    public ConteudoResponseDTO atualizar(
            ...
            @Valid @RequestBody ConteudoRequestDTO request,
            Principal principal) {
    ...
    public ConteudoResponseDTO concluir(
            @PathVariable Long id,
            @Valid @RequestBody ConcluirConteudoRequestDTO request,
            Principal principal) {
        return new ConteudoResponseDTO(
                service.concluir(id, request.getNivelDominio(), principal.getName()));
    }
}
