...
public class TecnologiaController {
    ...
    @PostMapping
    public ResponseEntity<TecnologiaResponseDTO> criar(
            @Valid @RequestBody TecnologiaRequestDTO request, Principal principal) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new TecnologiaResponseDTO(service.criar(request, principal.getName())));
    }
    ...
    @PutMapping("/{id}")
    public TecnologiaResponseDTO atualizar(
            @PathVariable Long id,
            @Valid @RequestBody TecnologiaRequestDTO request,
            Principal principal) {
