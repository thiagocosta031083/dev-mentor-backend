...
public interface PlanejamentoEstudoRepository extends JpaRepository<PlanejamentoEstudo, Long> {
    ...
    @Query(
            "select p from PlanejamentoEstudo p where p.tecnologia.id = :tecnologiaId "
                    + "and p.dataInicio <= :data and p.dataFim >= :data")
    Optional<PlanejamentoEstudo> findAtivoByTecnologiaId(
            @Param("tecnologiaId") Long tecnologiaId, @Param("data") LocalDate data);

    @Query(
            "select case when count(p) > 0 then true else false end from PlanejamentoEstudo p "
