...
public interface RegistroEstudoRepository extends JpaRepository<RegistroEstudo, Long> {
    ...
    @Query("select coalesce(sum(r.tempoMinutos), 0) from RegistroEstudo r where r.tecnologia.id = :tecnologiaId")
    Long somarMinutosPorTecnologia(@Param("tecnologiaId") Long tecnologiaId);

    @Query("select coalesce(sum(r.tempoMinutos), 0) from RegistroEstudo r " +
           "where r.tecnologia.id = :tecnologiaId and r.data between :inicio and :fim")
    Long somarMinutosPorTecnologiaEPeriodo(@Param("tecnologiaId") Long tecnologiaId,
