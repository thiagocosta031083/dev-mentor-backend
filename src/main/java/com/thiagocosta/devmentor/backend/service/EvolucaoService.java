...
public class EvolucaoService {
    ...
    public DashboardResponseDTO calcular(Long tecnologiaId, String email, LocalDate hoje) {
        ...
        long concluidos =
                todos.stream().filter(c -> c.getStatus() == StatusConteudo.CONCLUIDO).count();
        ...
        double horasPlanejadas =
                plano.map(PlanejamentoEstudo::getHorasPlanejadasTotais).orElse(0.0);
        double percentualEsperado =
                plano.map(p -> planoService.percentualEsperado(p, hoje)).orElse(0.0);
        Long minutos = registros.somarMinutosPorTecnologia(tecnologiaId);
        double horasRealizadas = (minutos == null ? 0 : minutos) / 60.0;
        double esforco =
                horasPlanejadas <= 0
                        ? 0
                        : Math.min(horasRealizadas / horasPlanejadas * 100.0, 100.0);
        ...
        return new DashboardResponseDTO(
                tecnologia.getNome(),
                arredondar(horasPlanejadas),
                arredondar(horasRealizadas),
                todos.size(),
                concluidos,
                arredondar(cobertura),
                arredondar(esforco),
                arredondar(pratica),
                arredondar(evolucao),
                arredondar(percentualEsperado),
                status);
    }
