...
public class ConteudoResponseDTO {
    ...
    private final StatusConteudo status;
    private final NivelDominio nivelDominio;

    public ConteudoResponseDTO(ConteudoPlanejado c) {
        id = c.getId();
        tecnologiaId = c.getTecnologia().getId();
        titulo = c.getTitulo();
        tipo = c.getTipo();
        peso = c.getPeso();
        status = c.getStatus();
        nivelDominio = c.getNivelDominio();
    }
