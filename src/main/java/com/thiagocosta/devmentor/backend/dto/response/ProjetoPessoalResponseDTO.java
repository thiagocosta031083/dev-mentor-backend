...
public class ProjetoPessoalResponseDTO {
    ...
    private final String proximoPasso;
    private final Long tecnologiaId;

    public ProjetoPessoalResponseDTO(ProjetoPessoal p) {
        id = p.getId();
        nome = p.getNome();
        stack = p.getStack();
        status = p.getStatus();
        proximoPasso = p.getProximoPasso();
        tecnologiaId = p.getTecnologia() == null ? null : p.getTecnologia().getId();
    }
